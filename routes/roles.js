...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { email, password, nom, prenom, zone_id, zone_nom, telephone } = req.body;

      // Déterminer la zone : Admin utilise sa zone, Super Admin peut choisir ou laisser vide
      let finalZoneId = zone_id;
      
      if (req.user.role === 'admin') {
        // Admin utilise automatiquement sa zone
        finalZoneId = req.user.zone_id;
      } else if (req.user.role === 'super_admin') {
        // Super Admin peut créer un agent sans zone (zone_id est optionnel)
        if (zone_id) {
          ...
          try {
            ...
            if (exactMatch) {
              ...
              console.log(`✅ Zone existante utilisée: ${exactMatch.nom} (${exactMatch.id})`);
            } else {
              ...
              console.log(`✅ Zone créée automatiquement: ${newZone.nom} (${newZone.id})`);
            }
          } catch (zoneError) {
            console.error('Erreur lors de la recherche/création de la zone:', zoneError);
      ...
      const errorMessage = error.message || 'Erreur serveur lors de la création de l\'agent.';
      res.status(500).json({ 
...
// Lister les agents (Super Admin voit tous, Admin voit ceux de sa zone)
router.get(
  '/agents',
  auth,
  requirePermission('manage_agents'),
  async (req, res) => {
    try {
      ...
      // Admin ne peut voir que les agents de sa zone
      if (req.user.role === 'admin') {
        filters.zone_id = req.user.zone_id;
      } else if (zoneId && req.user.role === 'super_admin') {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (admin.role !== 'admin') {
        return res.status(400).json({ message: 'Cet utilisateur n\'est pas un administrateur.' });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (agent.role !== 'agent_communal') {
        return res.status(400).json({ message: 'Cet utilisateur n\'est pas un agent communal.' });
      }
