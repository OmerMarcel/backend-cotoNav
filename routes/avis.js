...
router.get("/", async (req, res) => {
  try {
    ...
    const { data: avis, count } = await avisService.findAll(filters, {
      page,
      limit,
    });
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    if (!infrastructureId || !note) {
      return res
        .status(400)
        .json({ message: "infrastructureId et note sont requis." });
    }
...
router.patch("/:id", auth, async (req, res) => {
  try {
    ...
    const canModerate = ["admin", "super_admin", "moderateur"].includes(
      req.user.role,
    );
    const isOwner = avis.utilisateur_id === req.user.id;

    if (!isOwner && !canModerate) {
      return res
        .status(403)
        .json({ message: "Vous ne pouvez modifier que vos propres avis." });
    }

    if (req.body.approuve !== undefined && !canModerate) {
      return res
        .status(403)
        .json({ message: "Action de moderation non autorisee." });
    }
    ...
    const updatedAvis = await avisService.update(req.params.id, {
      ...
      approuve:
        req.body.approuve !== undefined ? !!req.body.approuve : undefined,
    });
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    if (
      avis.utilisateur_id !== req.user.id &&
      req.user.role !== "admin" &&
      req.user.role !== "super_admin"
    ) {
      return res
        .status(403)
        .json({ message: "Vous ne pouvez supprimer que vos propres avis." });
    }
