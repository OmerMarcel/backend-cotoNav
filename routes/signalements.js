...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!infrastructureId || !type || !description) {
      return res.status(400).json({ message: 'infrastructureId, type et description sont requis.' });
    }
...
router.patch('/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('❌ Erreur lors de la récupération du signalement:', findError);
    ...
    const signalement = await signalementService.update(signalementId, updateData);

    if (!signalement) {
      return res.status(404).json({ message: 'Signalement non trouvé après mise à jour.' });
    }

    // Envoyer une notification push aux utilisateurs (citoyens) selon le statut
    if (statut === 'resolu' || statut === 'valide' || statut === 'rejete' || statut === 'en_cours') {
      try {
        const infraName = signalement?.infrastructure?.nom || oldSignalement?.infrastructure?.nom || 'Infrastructure';
        let title, message;
        ...
        console.warn('⚠️ Notification push signalement traité échouée:', e.message);
      }
    }
