...
class PropositionService {
  ...
  transformToFlutterFormat(proposition) {
    ...
    const photos = this.normalizePhotos(
      proposition.photos || proposition.images,
    );
    ...
    return {
      // Garder aussi les champs originaux pour compatibilite
      ...proposition,
      id: proposition.id,
  ...
  async findAll(filters = {}, pagination = {}) {
    try {
      let query = supabase
        .from("propositions")
        .select(
          "*, propose_par:users!propose_par(*), modere_par:users!modere_par(*)",
          { count: "exact" },
        );
      ...
      try {
        ...
        console.error(
          "❌ Erreur de connexion Supabase dans findAll:",
          fetchError.message,
        );
        throw new Error(
          `Erreur de connexion à Supabase: ${fetchError.message}`,
        );
      }
      ...
      const transformedData = (data || []).map((prop) =>
        this.transformToFlutterFormat(prop),
      );

      return { data: transformedData, count: transformedData.length };
    } catch (error) {
      console.error(
        "❌ Erreur dans propositionService.findAll:",
        error.message,
      );
      throw error;
    }
  ...
  async findById(id) {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Erreur de connexion Supabase dans findById:",
          fetchError.message,
        );
        throw new Error(
          `Erreur de connexion à Supabase: ${fetchError.message}`,
      ...
      console.error(
        "❌ Erreur dans propositionService.findById:",
        error.message,
      );
      throw error;
    }
  ...
  async findByIdRaw(id) {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Erreur de connexion Supabase dans findByIdRaw:",
          fetchError.message,
        );
        throw new Error(
          `Erreur de connexion à Supabase: ${fetchError.message}`,
      ...
      console.error(
        "❌ Erreur dans propositionService.findByIdRaw:",
        error.message,
      );
      throw error;
    }
  ...
  async create(propositionData) {
    try {
      ...
      console.log(
        "📋 Données à insérer:",
        JSON.stringify(propositionData, null, 2),
      );
      ...
      try {
        ...
        console.error(
          "❌ Erreur de connexion Supabase dans create:",
          fetchError.message,
        );
        throw new Error(
          `Erreur de connexion à Supabase: ${fetchError.message}`,
      ...
      try {
        await notificationService.notify({
          ...
          message:
            `${data?.nom || "Infrastructure"} • ${data?.type || ""}`.trim(),
  ...
  async findByUserId(userId) {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Erreur de connexion Supabase dans findByUserId:",
          fetchError.message,
        );
        throw new Error(
          `Erreur de connexion à Supabase: ${fetchError.message}`,
      ...
      console.error(
        "❌ Erreur dans propositionService.findByUserId:",
        error.message,
      );
      throw error;
    }
  ...
  async update(id, updates) {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Erreur de connexion Supabase dans update:",
          fetchError.message,
        );
        throw new Error(
          `Erreur de connexion à Supabase: ${fetchError.message}`,
  ...
  async findByValidator(validatorId) {
    try {
      ...
      return (data || [])
        .map((prop) => this.transformToFlutterFormat(prop))
        .filter(Boolean);
    } catch (error) {
      console.error(
        "❌ Erreur dans propositionService.findByValidator:",
        error,
      );
      throw error;
    }
