-- ============================================
-- MIGRATION: Points -> Argent (Echanges)
-- ============================================
-- Ajoute les tables et fonctions pour l'echange de points en CFA.
-- Regle: 1 point = 0.5 F CFA (300 points = 150 F CFA)
-- Seuil minimum: 300 points

-- ============================================
-- 1. TABLE reward_exchanges
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_reward_exchanges_user_date
    ON reward_exchanges(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_reward_exchanges_status
    ON reward_exchanges(status);

-- ============================================
-- 2. FONCTION: Request exchange
-- ============================================
...
-- ============================================
-- 3. RLS (optionnel si service_role)
-- ============================================

ALTER TABLE reward_exchanges ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Utilisateurs voient leurs echanges"
    ON reward_exchanges FOR SELECT
    USING (user_id = auth.uid());

CREATE POLICY "Admins voient tous les echanges"
    ON reward_exchanges FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM users
...
CREATE POLICY "Systeme insere les echanges"
